...
def show_images_history(gr, opts, tabname, run_pnginfo, switch_dict):
    ...
    with gr.Row():
        renew_page = gr.Button('Renew Page|刷新页面', elem_id=tabname + "_images_history_renew_page")
        first_page = gr.Button('First Page|首页')
        prev_page = gr.Button('Prev Page|上一页')
        page_index = gr.Number(value=1, label="Page Index|页码")
        next_page = gr.Button('Next Page|下一页')
        end_page = gr.Button('End Page|尾页')
    with gr.Row(elem_id=tabname + "_images_history"):
        with gr.Row():
            with gr.Column(scale=2):
                ...
                with gr.Row():
                    delete_num = gr.Number(value=1, interactive=True, label="number of images to delete consecutively next|接下来要连续删除的图像数")
                    delete = gr.Button('Delete|删除', elem_id=tabname + "_images_history_del_button")
            with gr.Column():
                with gr.Row():
                    pnginfo_send_to_txt2img = gr.Button('Send to txt2img|发送到文本到图像')
                    pnginfo_send_to_img2img = gr.Button('Send to img2img|发送到图像到图像')
                with gr.Row():
                    with gr.Column():
                        img_file_info = gr.Textbox(label="Generate Info|创建信息", interactive=False)
                        img_file_name = gr.Textbox(label="File Name|文件名", interactive=False)
                with gr.Row():
                    # hiden items
...
def create_history_tabs(gr, opts, run_pnginfo, switch_dict):
    with gr.Blocks(analytics_enabled=False) as images_history:
        with gr.Tabs() as tabs:
            with gr.Tab("txt2img history|文本到图像历史"):
